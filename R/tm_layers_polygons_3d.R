...
#' @param cast.shadows Should the extruded polygons cast shadows? (`"maplibre"`/`"mapbox"` modes only). Passed to `fill_extrusion_cast_shadows`. The default `NULL` keeps the rendering engine's own default.
#' @param ambient.occlusion.intensity,ambient.occlusion.radius Ambient-occlusion intensity (0-1) and radius for the extruded polygons, adding soft contact shading in corners and crevices (`"maplibre"`/`"mapbox"` modes only). Passed to `fill_extrusion_ambient_occlusion_intensity` / `fill_extrusion_ambient_occlusion_radius`. The default `NULL` keeps the engine default.
#' @param vertical.gradient Should a vertical gradient be applied to the extrusion sides for a sense of depth? (`"maplibre"`/`"mapbox"` modes only). Passed to `fill_extrusion_vertical_gradient`. The default `NULL` keeps the engine default (`TRUE`).
#' @param emissive.strength Emissive strength of the extruded polygons, controlling how much they appear self-lit under the map's lighting (`"maplibre"`/`"mapbox"` modes only). Passed to `fill_extrusion_emissive_strength`. The default `NULL` keeps the engine default.
#' @export
